...
const LocationDay = () => {
  ...
  const getPosts = async () => {
    try {
      const res = await fetch(
        `/api/posts/${locationEventName}/${sidebarSelection}/view`
      );
  ...
  const displayedPosts = array.map((val, index) => {
    return (
      <>
        ...
        <Container className=>
          <div
            ...
            id={`postid-${val.id}`}
            style={{ margin: "auto" }}
            onMouseEnter={() => hoverEnterPost(val.id)}
            onMouseLeave={() => hoverLeavePost(val.id)}
            className="row mb-3 mt-3 ps-2 bg-dark"
          >
            <div
              id={`profile-card-${val.id}`}
              className="col-md-3 invisible  mb30"
            >
              <div className="card">
                <div className="row">
                  <img
                    ...
                    alt=""
                    className="col-4 m-2 rounded-circle"
                  ></img>
                  <h4 className="flex-column col-7 align-items-bottom">
                    <span>{val.profileName}</span>
                  </h4>
                  <div className="col-1"></div>
                </div>
                <div>
                  <ul>
                    <span>View {val.username}'s profile</span>
                  </ul>
                  <ul>
                    <span>{val.tag1}</span>
                  </ul>
            ...
            <div
              id={`post-card-${val.id}`}
              className="border-start me-2 col-md-9"
            >
              <div className="card">
                <div>
                  <ul>
          ...
          {index === array.length - 1 ? console.log(index) : null}
        </Container>
          </Card.ImgOverlay>
      </>
    );
