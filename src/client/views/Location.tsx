...
const Location = () => {
  ...
  const dateArray = getDaysArray(
    new Date(`1971-01-01 ${time}`),
    new Date(`${date2} ${time}`)
  );

  const dateArray2: Date[] = getDaysArray(
    new Date(`2016-01-01 ${time}`),
    new Date(`${date2} ${time}`)
  );
  ...
  const bruh = ({ date, view }) =>
    view === "month" &&
    date.toDateString() ===
      datesToAddContentTo.find((dDate) => dDate == date.toDateString()) ? (
      <div className="p-0 highlight customTile">🔥</div>
    ) : null;
  ...
  const bruhFunc = (value: Date) => {
    ...
    if (
      value.toDateString() ===
      datesToAddContentTo.find((dDate) => dDate == value.toDateString())
    ) {
          ...
          <div className="ms-2 card">
            <ul className="list-group  list-group-flush">
              ...
              <li className="list-group-item bg-dark  pl-2">
                <Link
                  to={`/${locationEventName}/${deconstructedMonth}-${deconstructedDay}-${deconstructedYear}/post`}
                >
                  <button
                    id="makeButton"
              ...
              <li className="list-group-item bg-dark pl-2">
                <Link to={`/${locationEventName}/${sidebarSelection}`}>
                  <button
                    id="viewButton"
                    type="button"
